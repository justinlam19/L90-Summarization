...
class Preprocesser:
    ...
    def vectorize_sentence(self, tokens):
        if len(tokens) > 0:
            word_vectors = np.array(
                [self.model[word] for word in tokens if word in self.model.key_to_index]
            )
    ...
    def article_for_rnn(self, article: list[str]):
        ...
        cosine_similarities = self.normalize(
            np.array([self.cosine_similarity(s, mean_vector) for s in sentence_vectors])
        )

        inv_freqs = self.normalize(
            np.array([self.inv_freq(tokens) for tokens in tokens_list])
        )
        ...
        return [
            np.reshape(v, (4, -1))
            for v in np.vstack((cosine_similarities, inv_freqs, lengths, positions)).T
        ]
